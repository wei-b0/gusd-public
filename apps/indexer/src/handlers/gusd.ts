...
ponder.on("GUSD:Minted", async ({ event, context }) => {
...
ponder.on("GUSD:Redeemed", async ({ event, context }) => {
  const keys = eventKeys(event, context.chain.id);
...
ponder.on("GUSD:FeesUpdated", async ({ event, context }) => {
...
ponder.on("GUSD:Transfer", async ({ event, context }) => {
  const keys = eventKeys(event, context.chain.id);
