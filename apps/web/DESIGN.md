...
- The status line is source-aware: `Oracle index · trading simulated` in oracle mode, `Demo · all data simulated` in mock mode.
...
- One demo admission, everywhere, on the status line: "Demo · all data simulated" (mock mode) or "Oracle index · trading simulated" (oracle mode) — surfaces do not re-confess; only unresolved *product concepts* carry a PROTOTYPE tag, and Index reads carry provenance chips (data truth, not confessions)
...
- **The dominant plate:** the chart is the largest object — 320px tall on mobile; 50vh (discovery) / 56vh (desk) at ≥1024px, minimum 360–380px.
- **Rails:** the trading desk is deliberately dense — a 230px markets rail (markets, position, underlying GPU), fluid plate, 310px trade-and-index rail; beneath the plate run premium/discount history beside market statistics, then full-width Index sources and the activity zone (this market's tape, then the all-market tape); gUSD binds a 320px rail (balances + earning ledger). The desk's markets-rail entries are links to each market's own desk (`/terminal/[asset]`, `aria-current` marks the bound one) — switching markets changes the route, so the URL always names the market on screen. Below 1024px everything stacks.
...
The TuiPanel plate: amber numbered head, mono meta stating the selected grain ("1m candles · UTC" … "1w candles · UTC" — the selector label IS the candle grain, mirroring the server's series plan; the demo venue layer still synthesizes hourly bars, so only that mode reads "hourly candles · UTC"), the interval selector on the head's right (11px mono, bottom-bordered, the active interval amber: 1m 5m 15m 30m 6h 12h 1d 1w — TradingView grain idiom), a caption foot ("Band shows the premium / discount gap" under a venue leg). Without a venue leg (oracle mode) the plate drops the band and captions "Benchmark series · candles from the canonical benchmark history" — the candles are the benchmark's own series, server-bucketed, never simulated.
...
- **Candles:** solid bright-green up, solid red down — body and wick in the move color, **borderless** (`borderVisible: false` — at dense intervals a 1px border turns a candle into a filled block and eats the open/close gap). A **dotted last-price line** tagged on the axis (`priceLineVisible`, `LineStyle.Dotted`, `lastValueVisible`) carries the live close; its color follows the last bar's direction (green up, red down) and re-engraves as ticks land. `priceFormat` precision 4 / minMove 0.0001 — the benchmark moves in fractions of a cent, and the default 2dp format rounds them into a flat line.
- **The Index line:** only under a venue leg — when the candles ARE the Index (oracle mode) the wire is not drawn at all: one series, one price. With a venue leg the Index runs as a 1px cyan line, no crosshair marker; the legend labels it "Index" in cyan.
...
- **Time axis:** `tickMarkFormatter = fmtAxisTime(t, range)` — HH:MM on the minute grains (1m/5m/15m/30m), "DD MON HH:00" on 6h/12h, "DD MON" on 1d/1w. Full range with breathing room, and the **candle pitch is capped** (~12px): a short series (a young benchmark, a wide interval) spreads its slack ahead of the data instead of stretching a handful of bars into pane-wide slabs; the live candle never clips the seam.
...
### Index sources (desk panel 06; Oracle page panel 02)
...
### Oracle (`/oracle`)
The reference layer — the old Index and Data surfaces compressed into one section, panels numbered in pipeline order: **01 Index board** (benchmarks per GPU-hour class, cyan figures, provenance chip beside each Index read; the market-price and premium columns render only where a market layer prices the rows — the board stays pure reference infrastructure in oracle mode), **02 Index history** (the wire alone, benchmark + range switchers), **03 Methodology** (the provider→panel→Index pipeline diagram, honest-machine caveat, `methodology 0.1.0` in the meta), **04 Panel health** (publication `#hashprefix`, updated age, live sources; coverage prints `—` — the oracle does not publish it), **05 Interface catalog** (status-as-shipped rows: LIVE chips on the endpoints the oracle serves today — Index feed REST, provider panel REST, SSE stream, health — PROTOTYPE on the web's market-snapshot seam, PLANNED on history datasets and protocol RPC), **06 Sample payload** (the live last-fetched `CandidateDto` for H100 in oracle mode; a static wire-shaped sample in mock mode), **07 Access** (points to `/protocol` — Protocol's only organic doorway from a surface). `/oracle/[asset]` is the cyan-led benchmark sheet: provider panel, weighting bars, data note, and an amber depth note when the real publication history is shallower than the selected range. Index stays the benchmark *name* (H100 Index, gUSD Index); Oracle is the *section* that observes inputs, produces the benchmarks, and publishes them. `/index`, `/index/[asset]`, and `/data` 308-redirect here.
...
- **Provenance chips are data truth, not confessions:** LIVE/STALE/WITHHELD/FROZEN/UNAVAILABLE state the oracle's publication status and age for an Index read. They sit beside every oracle-backed Index read (markets table, asset heads, oracle board) and never decorate; the mock data source renders no chip — its admission lives on the status line alone.
