...
"Basis" survives only in advanced technical contexts (Protocol, Data, chart footnotes) where it names the concept precisely. Asset codes (H100, L40S) stay bare in figures and command-line input but never stand alone as page titles — pages title the pair.
...
**The command line (signature).** An amber `>` prompt (15px bold), a bold amber lowercase-tolerant input (13px, 64px wide / 112px at ≥640px, placeholder "terminal H200" in dim), and the GO key in reverse amber. It speaks **product names, not codes**: `markets`, `terminal`, `oracle`, `gusd`, `portfolio`, `protocol` (case-insensitive; `board` aliases markets; `index` opens the Oracle's Benchmarks tab; `data` opens its Developers tab), each accepting a bare asset argument (`terminal H200`; `oracle H200` and `index H200` deep-link the Benchmarks tab with that benchmark's receipt open); bare `terminal` opens the default desk (H100); `gusd`/`sgusd`/`mint`/`earn`/`vaults` all land on `/gusd`; a lone asset code (`H200`) routes to that market's desk, as does `markets H200` — the bare `markets` command is the board. A rejected input prints `UNKNOWN COMMAND "X" — TRY MARKETS · TERMINAL · ORACLE` in amber slug, `role="status" aria-live="polite"` — inline at ≥1024px, as a strip under the bar below — and the line self-clears after ~3.2s. The caret is amber.
