...
contract InvariantTest is Test, Deployers, IWorld {
    ...
    function setUp() public {
        ...
        s_issuance = new GPUIssuance(
            IERC20(address(s_gusd)), IGPUPriceOracle(address(s_oracle)), address(s_ledger), address(this)
        );
        ...
        initReserve = s_issuance.gpuReserve(H100);
    ...
    /// @notice INVARIANT 3: every s_issuance put required gUSD into the reserve.
    function invariant_reserveMatchesIssuanceAccounting() public view {
        assertEq(s_issuance.gpuReserve(H100), initReserve + gBase);
        assertGt(s_issuance.gpuReserve(H100), 0);
        assertEq(s_gusd.balanceOf(address(s_issuance)), s_issuance.gpuReserve(H100));
    }

    /// @notice INVARIANT 4+5: secondary trades never touch primary reserves;
    ///         LP capital is separate from s_issuance reserves (gUSD-only).
    function invariant_secondaryTradesLeaveReservesUntouched() public view {
        assertEq(s_issuance.gpuReserve(H100), initReserve + gBase);
        assertEq(s_gusd.balanceOf(address(s_issuance)), s_issuance.totalIssuanceReserve());
    }

    /// @notice INVARIANT 6: no s_oracle-NAV redemption — s_oracle price changes
    ///         never alter reserve accounting (handler sets arbitrary prices).
    ...
    function invariant_revenueConserved() public view {
        ...
        assertEq(
            inLedger + s_ledger.totalToVault() + s_ledger.totalToTreasury(), initLedgerReceived + gFee + gHarvested
        );
    }
